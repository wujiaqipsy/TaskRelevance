...
plot_rt_m <- plot_cond_rt_m +
  ...
  ggplot2::geom_line(data = df.clas.rt.plot,
                     ...
                     color="#000000",
                     alpha=0.05) +
  ggplot2::scale_x_continuous(breaks = c(1, 2, 3), labels = c("自我优先", "朋友优先", "生人优先")) +
...
# Priority：因子，自我优先为基线，取值为自我优先、朋友优先、生人优先
# subj_idx：数值
```
...
# X3=朋友优先的数据[带入(0, 1, 0)]；X4=生人优先的数据[带入(0, 0, 1)]
# 自我优先（0，0，0）
# 朋友优先（0，1，0）
# 生人优先（0，0，1）
```
...
df_rt_pop_diff_wide <- df_rt_pop %>%
  ...
  dplyr::mutate(
    ...
    # 以下是我加的差异的差异，大的-小的
    diff_diff_SF_PSF = diff_SF_PS - diff_SF_PF, # (RT[自我优先] - RT[朋友优先])[自我-朋友]
    diff_diff_SF_PSStr = diff_SF_PS - diff_SF_PStr,# (RT[自我优先] - RT[生人优先])[自我-朋友]
    diff_diff_SStr_PSF = diff_SStr_PS - diff_SStr_PF, # (RT[自我优先] - RT[朋友优先])[自我-生人]
    diff_diff_SStr_PSStr = diff_SStr_PS - diff_SStr_PStr,# (RT[自我优先] - RT[生人优先])[自我-生人]
  ) %>%
  dplyr::select(`.chain`, `.iteration`, `.draw`,
...
# Regression Coefficients:
#                                        Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
# Intercept                                  1.31      0.09     1.13     1.49 1.00     4433     7311
# Identityfriend                            -0.04      0.09    -0.21     0.13 1.00     9226    11234
# Identitystranger                           0.00      0.06    -0.12     0.13 1.00    14834    12659
# Priorityfriend_goal                        0.18      0.14    -0.10     0.46 1.00     7310    10185
# Prioritystranger_goal                      0.28      0.18    -0.06     0.63 1.00     8159     9989
# Identityfriend:Priorityfriend_goal         0.11      0.13    -0.15     0.36 1.00    11014    11171
# Identitystranger:Priorityfriend_goal      -0.06      0.10    -0.26     0.14 1.00    16732    12728
# Identityfriend:Prioritystranger_goal      -0.09      0.15    -0.38     0.20 1.00    10409    12169
# Identitystranger:Prioritystranger_goal     0.08      0.15    -0.22     0.39 1.00    13484    12913
```
