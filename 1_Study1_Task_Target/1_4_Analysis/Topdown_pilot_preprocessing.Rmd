...
```{r warning=FALSE}
df_raw <- list.files(file.path("../4_4_2_RawData"), pattern = "exp3_.*\\.csv$") %>%
  # 对读取到的所有.out文件x都执行函数read.table
  lapply(function(x) read.csv(file.path("../4_4_2_RawData", x), header = TRUE)) %>%
    # 读取被试人口学信息
...
write_excel_csv(df.match.acc,file = "./4_4_3_1_clean_data/exp3_matching_acc.csv")
...
write_excel_csv(df.match.rt, "./4_4_3_1_clean_data/exp3_matching_rt.csv")
```
...
write_excel_csv(df.acc,file = "./4_4_3_1_clean_data/exp3_classify_acc.csv")
```
...
write_excel_csv(df.rt, "./4_4_3_1_clean_data/exp3_classify_rt.csv")
```
