...
var info = []   // 存储被试信息：ID
var ShapeColorMap = new Map();
var ShapeLabelMap = new Map();
...
function getLabelColorTimelineVariables(colored_shapes) {
    ...
    colored_shapes.forEach(imagePath => {
        ...
        // console.log('当前图片匹配的标签paired_label', paired_label)
        // console.log('当前图片呈现的标签pres_label', pres_label)      
...
console.log('根据被试ID随机的图形', shape_images)
// 图形与标签配对
...
console.log('ShapeColorMap', ShapeColorMap);
console.log('ShapeLabelMap', ShapeLabelMap);
console.log('key', key);
console.log('最终输出的pres_labels', pres_labels);
console.log('任务无关条件时间线变量', TIR_timeline_variables);
...
var Tr_instr = {
    ...
    pages: function () {
        ...
        return [`<p class='header' style = 'font-size: 35px'>实验说明：</p>
            ...
            `<p class='footer' style='font-size: 35px; line-height: 30px;'>首先进行图形-标签匹配任务。</p>
...
var TR_prac = {
    timeline: [
        {   // 单个试次
            ...
            trial_duration: 2500,
            on_start: function () {
                console.log('pres_labels of this trial:', jsPsych.timelineVariable('pres_label'));
...
var TR_reprac_instr = { //在这里呈现文字回顾，让被试再记一下
    type: jsPsychInstructions,
    pages: function () {
        let start = "<p class='header' style='font-size:35px; line-height:30px;'>请您努力记下下列图形-标签的对应关系，并再次进行练习。</p>",
...
// 休息指导语
let TR_resid_block_numb = 1;// 此处填入总block数量-1，比如总数量是3，那么值就需要是2
let TR_main_rest = {
    type: jsPsychHtmlButtonResponse,
    stimulus: function () {
...
var TIR_prac = {
    timeline: [
        {   // 单个试次
            ...
            trial_duration: 2500,
            on_start: function () {
                console.log('pres_labels of this trial:', jsPsych.timelineVariable('pres_color'));
                console.log('paired_labels of this trial:', jsPsych.timelineVariable('paired_color'));
                console.log('correct_response of this trial:', jsPsych.timelineVariable('identify'));
            },
            on_finish: function (data) {
    ...
    randomize_order: true,   //6
    repetitions: 3,   // 需要与任务相关条件练习次数一样，必须设置成3
    on_load: () => {
        $("body").css("cursor", "none");
    },
    on_finish: function () {
...
var TIR_reprac_instr = { //在这里呈现文字回顾，让被试再记一下
    type: jsPsychInstructions,
    pages: function () {
        let start = "<p class='header' style='font-size:35px; line-height:30px;'>请您努力记下下列图形-颜色对应关系，并再次进行练习。</p>",
            middle = "<p class='footer' style='font-size:35px; line-height:30px;'>如果对本实验还有不清楚之处，请立即向实验员咨询。</p>",
            end = "<p style='font-size:35px; line-height:30px;'>如果您明白了规则：</p><p style='font-size:35px; line-height:30px;'>请按 继续 进入练习</p><div>";
        let tmpI = "";
...
var TIR_main = {
    timeline: [
        {   // 单个试次
            ...
            on_finish: function (data) {
                ...
                data.paired_colors = jsPsych.timelineVariable("paired_color");   // 配对的颜色
                data.ismatch = data.pres_colors == data.paired_colors;   // 呈现的颜色与学习的颜色是否匹配
                data.condition = "TIR_main";   // 标记为练习阶段
            }
        },
        {   // 每个试次后反馈
            type: jsPsychHtmlKeyboardResponse,
    ...
    repetitions: 3,   // 需要保证每个实验条件优势试次有60个
    on_load: () => {
        $("body").css("cursor", "none");
    },
    on_finish: function () {
...
// 休息指导语
let TIR_resid_block_numb = 1;// 此处填入总block数量-1，比如总数量是3，那么值就需要是2
let TIR_main_rest = {
    type: jsPsychHtmlButtonResponse,
    stimulus: function () {
        let totaltrials = jsPsych.data.get().filter(
        ...
        return `
                    <p>图形-标签匹配任务中，您还剩余${TIR_resid_block_numb}组实验</p>
                    <p>现在是休息时间，当您结束休息后，您可以点击 结束休息 按钮 继续</p>
                    <p>建议休息时间还剩余<span id="iii">60</span>秒</p>`
    },
    choices: ["结束休息"],
