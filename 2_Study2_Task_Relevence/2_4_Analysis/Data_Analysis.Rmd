---
title: "DataAnalysis"
author: "wujiaqi"
date: "2025-04-25"
output: html_document
---

# 配置环境与R包
```{r}
###################### 配置环境与R包 ######################

# 配置环境
if (.Platform$OS.type == 'windows') {
        Sys.setlocale(category = 'LC_ALL','en_US.UTF-8')   # 使用UTF-8编码
} 

setwd("C:/1_Postgraduate/TaskRelevance/2_Study2_Task_Relevence/2_4_Analysis")   # 设置工作目录

set.seed(42)   # 随机种子，确保结果可复现

rm(list = ls())   # 清除当前工作环境中所有对象（变量、函数等）

curDir = getwd()   # 获取当前工作路径

figDir = file.path(curDir, 'figures')   # 设置图片存储地址

# 安装cmdstanr作为后端
if (!require(cmdstanr)){
        install.packages("cmdstanr", repos = c('https://stan-dev.r-universe.dev', getOption("repos")))
        library(cmdstanr)
}
set_cmdstan_path('C:/softwares/cmdstan-2.36.0')



# 配置R包
if (!require(pacman)){
        install.packages("pacman")
        library(pacman)
}

## 使用pacman管理R包
pacman::p_load(
  'knitr',
  'here',         # 选择路径
  'tidyverse',    # 数据处理
  'ggplot2',      # 数据可视化
  'brms',         # 贝叶斯统计
  'bayesplot',    # 贝叶斯绘图
  'tidybayes',    # 贝叶斯统计
  'ggridges',     # 绘制山脊图
  'patchwork',    # 将多个图拼接在一起
  "papaja",       # 生成符合APA格式的文档
  'cmdstanr',
  'see',   # 后验推断结果可视化
  'bayestestR', 
  'parameters',
  'performance',
  'report'   # 结果报告
               )

theme_set(theme_modern())

```

# 数据准备
## 数据读取
```{r}
###################### 数据准备 ######################

df_raw <- list.files(file.path("../2_3_RawData"), pattern = "exp2_.*\\.csv$") %>%
  # 对读取到的所有.out文件x都执行函数read.table
  lapply(function(x)
    read.csv(file.path("../2_3_RawData", x), header = TRUE)) %>%
  # 读取被试人口学信息
  lapply(
    function(df)
      mutate(
        df,
        # subj_idx = as.numeric(jsonlite::fromJSON(response[5])$Q0),
        gender = jsonlite::fromJSON(response[5]),
        year = jsonlite::fromJSON(response[6])$Q0,
        education = jsonlite::fromJSON(response[7])$Q0,
        dist = view_dist_mm[9],
        rt = as.numeric(rt),
        success = as.character(success),
        timeout = as.character(timeout),
        correct = as.character(correct),
        ismatch = as.character(ismatch),
        correct = gsub("TRUE", "true", correct),
        ismatch = tolower(ismatch),
      )
  ) %>%
  bind_rows()
```

## 数据清洗
```{r}
###################### 数据清洗 ######################

df <- df_raw %>%
  # 排除预实验数据
  # filter(!(subj_idx %in% 1:30))%>%
  select(
    subj_idx,
    gender,       # 男0女1
    year,
    rt,
    correct_response,
    correct,
    key_press,
    shapes,       # 配对的标签（图形的社会属性
    pres_labels,    # 呈现的标签
    pres_shapes,    # 呈现的图形
    pres_colors,    # 呈现的颜色
    paired_colors,    # 配对的颜色
    ismatch,
    condition,    # 图形-标签/呈现颜色-配对颜色 是否匹配
    trial_type,    # task types
  ) %>%
  # 选择正式实验试次
  dplyr::filter(trial_type == "psychophysics" &
                  condition != "TR_prac" &
                  condition != "TIR_prac") %>%
  # 再次剔除无关变量
  mutate(
    year = as.numeric(year),
    year = 2025 - year,
    acc = ifelse(correct == "true", 1, 0),
    ismatch = ifelse(ismatch == "true", 1, 0),
  ) %>%
  select(
    -trial_type, 
    -correct,
    -correct_response)

head(df)
  
df

# length(unique(df$subj_idx))   # 查看被试总数

```

## 数据描述
```{r}
###################### 数据描述 ######################

# 正确率低于0.7的被试
df.excld.sub <-  df %>%
        dplyr::group_by(subj_idx) %>%
        dplyr::summarise(N = length(acc),                   
                         N_crrct = sum(acc),
                         ACC = sum(acc)/length(acc)) %>%
        dplyr::filter(ACC < 0.7) %>%                        
        dplyr::select(subj_idx)

# 无效试次占比
df.invalid_trial_rate <- df %>%
        dplyr::filter(!(subj_idx %in% df.excld.sub$subj_idx)) %>%   # 选出正确率高于0.7的被试
        dplyr::summarize(rate = length(rt[!(rt >= 200 & rt<=1500 & acc == 1)])/length(rt))  # 排除不正确试次以及正确但rt不在200-1500ms的试次 


# 有效数据框
df.v <- df %>%
        dplyr::filter(!(subj_idx %in% df.excld.sub$subj_idx))   # 排除正确率低于0.7的被试
df.v

```

```{r}
# 基于有效被试的人口学信息统计
df.basic <- df.v %>%
        dplyr::select(subj_idx, year, gender) %>%
        dplyr::distinct(subj_idx, .keep_all = TRUE) %>%     # 根据subject去重，保留完整行
        dplyr::summarise(N = length(subj_idx),
                         Nf = length(gender[gender == "1"]),   # 计算女性人数
                         Nm = length(gender[gender == "0"]),   # 计算男性人数
                         Age_mean = round(mean(year,na.rm=TRUE),2),
                         Age_sd = round(sd(year,na.rm=TRUE),2)
        )
```


## 据框格式整理
### 方差分析
```{r}
###### 获取每个被试在每个条件下的平均反应时 适用R ######
df.rt_m <- df.v %>%
        dplyr::filter(acc == 1) %>%
        dplyr::group_by(subj_idx, condition, shapes, ismatch) %>%
        dplyr::summarise(RT_m = mean(rt),
                         RT_SD = sd(rt),
                         RT_SE = RT_SD/sqrt(n()-1),
                         Ntrial = length(rt),
                         .groups = 'drop') 

df.acc <- df.v %>%
        # dplyr::filter(subj_idx == 26) %>% 
        # dplyr::group_by(subj_idx)%>%# 发被试费用
        dplyr::group_by(subj_idx, condition, shapes, ismatch) %>%
        dplyr::summarise(N = length(acc),      
                         N_crrct = sum(acc),
                         ACC = sum(acc)/length(acc),
                         .groups = 'drop')


# 计算效率值η
df.η <- df.rt_m %>%
  inner_join(df.acc, by = c("subj_idx", "condition", "shapes", "ismatch")) %>%
  mutate(η = ifelse(ACC == 0, NA, RT_m / ACC)) %>%
  select(subj_idx, condition, shapes, ismatch, η)


# 信号检测论d prime
df.dprime <- df.v%>%
    group_by(subj_idx, condition, shapes) %>%   # 输入除ismatch外另外两个自变量
    summarise(
      match_trials = sum(ismatch == 1),   # 匹配试次总数
      hits = sum(ismatch == 1 & acc == 1),   # 匹配情况下，正确识别为匹配
      nonmatch_trials = sum(ismatch == 0),   # 不匹配试次总数
      false_alarms = sum(ismatch == 0 & acc == 0),   # 不匹配情况下，错误识别为匹配
      .groups = 'drop'
    ) %>%
    mutate(
      # 处理极端值：# 如果击中为1，则调整为1 - 1/(2N)；如果为0则调整为1/(2N)，否则按照hits/match_trials计算
      HR = ifelse(hits/match_trials == 1, 1 - 1/(2*match_trials),   
                 ifelse(hits/match_trials == 0, 1/(2*match_trials), hits/match_trials)),
      FAR = ifelse(false_alarms/nonmatch_trials == 1, 1 - 1/(2*nonmatch_trials),
                  ifelse(false_alarms/nonmatch_trials == 0, 1/(2*nonmatch_trials), false_alarms/nonmatch_trials)),
      # 计算d prime
      dprime = qnorm(HR) - qnorm(FAR)
    )
  
# write_excel_csv(df.rt_m, file = file.path(here('2_Study2_Task_Relevence', '2_4_Analysis'), "exp2_rt_m.csv"))
```

```{r}
###### 获取每个被试在每个条件下的平均反应时 适用JASP ######

df.v.jasp <- df.v%>%
  mutate(
    condition = case_when(condition == "TR_main" ~ "TR", condition == "TIR_main" ~ "TIR", ),
    shapes = case_when(
      shapes == "自我" ~ "self",
      shapes == "朋友" ~ "friend",
      shapes == "生人" ~ "stranger",
    ),
    ismatch = case_when(ismatch == "1" ~ "match", ismatch == "0" ~ "mismatch", )
  )



df.rt.jasp <- df.v.jasp %>%
  dplyr::filter(acc == 1) %>%
  dplyr::group_by(subj_idx, condition, shapes, ismatch) %>%
  dplyr::summarise(RT_m = mean(rt), .groups = 'drop') %>%
  pivot_wider(names_from = c("condition", "shapes", "ismatch"),
              values_from = "RT_m") 



df.acc.jasp <- df.v.jasp %>%
  dplyr::group_by(subj_idx, condition, shapes, ismatch) %>%
  dplyr::summarise(ACC = sum(acc) / length(acc), .groups = 'drop') %>%
  pivot_wider(names_from = c("condition", "shapes", "ismatch"),
              values_from = "ACC") 



df.η.jasp <- df.η %>%
    select(subj_idx, condition, shapes, ismatch, η)%>%
    mutate(
    condition = case_when(condition == "TR_main" ~ "TR", condition == "TIR_main" ~ "TIR", ),
    shapes = case_when(
      shapes == "自我" ~ "self",
      shapes == "朋友" ~ "friend",
      shapes == "生人" ~ "stranger",
    ),
    ismatch = case_when(ismatch == "1" ~ "match", ismatch == "0" ~ "mismatch" ))%>%
  pivot_wider(names_from = c("condition", "shapes", "ismatch"),
              values_from = "η")


df.dprime.jasp <- df.dprime%>%
  select(subj_idx, condition, shapes, dprime)%>%
    mutate(
    condition = case_when(condition == "TR_main" ~ "TR", condition == "TIR_main" ~ "TIR", ),
    shapes = case_when(
      shapes == "自我" ~ "self",
      shapes == "朋友" ~ "friend",
      shapes == "生人" ~ "stranger",
    ) )%>%
  pivot_wider(names_from = c("condition", "shapes"),
              values_from = "dprime")
  
# jasp_path <- "C:/1_Postgraduate/TaskRelevance/2_Study2_Task_Relevence/2_4_Analysis/Jasp"
# write_excel_csv(df.rt.jasp, file = file.path(here('2_Study2_Task_Relevence', '2_4_Analysis', 'Jasp'), "exp2_rt_jasp.csv"))
# write_excel_csv(df.acc.jasp, file = file.path(here('2_Study2_Task_Relevence', '2_4_Analysis', 'Jasp'), "exp2_acc_jasp.csv"))
# write_excel_csv(df.dprime.jasp, file = file.path(here('2_Study2_Task_Relevence', '2_4_Analysis', 'Jasp'), "exp2_dprime_jasp.csv"))
write_excel_csv(df.η.jasp, file = file.path(here('2_Study2_Task_Relevence', '2_4_Analysis', 'Jasp'), "exp2_η_jasp.csv"))
```

# bootstrap
```{r}
# Step1: 根据自变量分组，将每个组合下的RT与ACC变为一个数据点(RT,ACC)

# 1.1 合并RT与ACC数据框，一行表示一个被试
df_combined <- df.rt_m %>%
  left_join(df.acc, by = c("subj_idx", "condition", "shapes", "ismatch")) %>%
  select(subj_idx, condition, shapes, ismatch, RT_m, ACC)



# 1.2 获取每个条件组合下的数据框列表
df_list <- df_combined %>%
  group_by(condition, shapes, ismatch) %>%
  group_split() %>%
  setNames(
    map_chr(., ~ paste(.x$condition[1], .x$shapes[1], .x$ismatch[1], sep = "_"))
  ) %>%
  map(~ select(., RT_m, ACC))


# names(df_list)
# df_list[["TIR_main_朋友_0"]]   # 访问某个实验条件下的数据框




# Step2: 进行一次有放回重采样，采样次数为单个条件的样本量；计算重采样样本的RT与ACC均值；重复k次

# 2.1 定义重采样函数
bootstrap_sample <- function(data) {
  n <- nrow(data)   # 一行一个被试，获取实验条件下的样本量
  indices <- sample(1:n, replace = TRUE, size = n)   # 从1：n中进行有放回抽样，抽取size次，返回一串索引
  bootstrap_sample <- data[indices, ]   # 通过索引获取重采样数据，获取索引所在行，组合为重采样数据框1
  return(bootstrap_sample)
}



# 2.2定义函数对每个条件数据点(RT,ACC)进行重采样与均值计算
run_bootstrap_analysis <- function(data_list, k = 2000) {
  results <- list()
  set.seed(42)
  
  for (condition_name in names(data_list)) {
    data <- data_list[[condition_name]]   # 获取每个实验条件的数据点
    
    mean_rt <- numeric(k)
    mean_acc <- numeric(k)
    
    for (i in 1:k) {
      bootstrap_sample_data <- bootstrap_sample(data)   # 进行一次重采样
      mean_rt[i] <- mean(bootstrap_sample_data$RT_m)   # 计算单次重采样样本的RT均值
      mean_acc[i] <- mean(bootstrap_sample_data$ACC)   # 计算单次重采样样本的ACC均值
    }
    
    results[[condition_name]] <- data.frame(   # 存储每个实验条件下的重采样均值结果
      condition = condition_name,
      mean_rt = mean_rt,
      mean_acc = mean_acc
    )
  }
  
  return(bind_rows(results))
}



# 2.3 使用函数
bootstrap_results <- run_bootstrap_analysis(df_list, k = 2000)
```



```{r}
# Step3: 以acc为X轴，RT为Y轴，按匹配情况分为2个子图，子图内图例为标签，统一坐标轴范围和刻度

# 3.1 整理用于画图的数据格式
bootstrap_results_plot <- bootstrap_results %>%
  mutate(
    condition_clean = str_replace(condition, "_main", "")  # 去掉_main
  ) %>%
  separate(condition_clean,
           into = c("condition", "shapes", "ismatch"),
           sep = "_") %>%
  mutate(
    ismatch = factor(
      ifelse(ismatch == "0", "不匹配", "匹配"),
      levels = c("匹配", "不匹配")),
    condition = factor(
      case_when(condition == "TIR" ~ "任务有关", condition == "TR" ~ "任务无关"),
      levels = c("任务有关", "任务无关")),
    shapes = factor(shapes, levels = c("自我", "朋友", "生人"))
  ) %>%
  relocate(mean_rt, mean_acc, .after = last_col())

bootstrap_results_plot



# 3.2 画图
plot <- ggplot(bootstrap_results_plot, aes(x = mean_acc, y = mean_rt, color = shapes)) +
  geom_point(alpha = 0.6, size = 2) +  # 添加透明度使重叠点可见
  facet_grid(ismatch ~ condition) +  # 按condition和ismatch分面
  labs(
    title = "正确率与反应时分布，按任务条件和匹配情况分组",
    x = "正确率", 
    y = "反应时(ms)",
    color = "身份"
  ) +
  ylim(500, 1000) +
  xlim(0.5, 1.0) +
  theme_apa() +
  theme(
    legend.position = "bottom",
    strip.text = element_text(face = "bold"),
    panel.spacing = unit(1, "lines")  # 增加面板间距
  )

print(plot)

```


### 配对样本t检验
```{r}
###### 获取每个被试在每个条件下的平均反应时 适用JASP 配对样本t检验 ######

# unique_conditions <- unique(df.acc$condition)   # 获取 condition 列的所有唯一值
# 
# # ACC数据
# for (cond in unique_conditions) {
#         filtered_data <- df.acc.jasp %>% filter(condition == cond)
#         file_name <- paste0("exp2_", cond, "_acc", ".csv")   # 构造文件名
#         write_excel_csv(filtered_data, file_name)   # 将筛选后的数据保存为 CSV 文件
# }
# 
# # RT数据
# for (cond in unique_conditions) {
#         filtered_data <- df.rt.jasp %>% filter(condition == cond)
#         file_name <- paste0("exp2_", "rt_", cond, ".csv")
#         write_excel_csv(filtered_data, file = file.path(base_path, file_name))
# }
```

### 贝叶斯层级模型
```{r}
###################### 用于贝叶斯层级模型的数据 trial level ######################

# shape-label matching task
df.bayes <- df.v %>%
        dplyr::filter(!is.na(key_press)) %>%
        dplyr::filter(rt >= 200 & rt<=1500)%>%
        dplyr::mutate(
                saymatch = ifelse((ismatch == '1' & acc == 1) |
                                          (ismatch == '0' & acc == 0), 1, 0),
                shapes = factor(shapes, levels = c("自我", "朋友", "生人")),
                condition = case_when(
                  condition == "TR_main" ~ "TaskRelevant",
                  condition == "TIR_main" ~ "TaskIrrelevant",
                ),
                condition = factor(condition, levels = c("TaskRelevant", "TaskIrrelevant")),
        )%>%
        select(subj_idx, condition, shapes, acc, rt, ismatch, saymatch, key_press)
df.bayes
```

# 数据初步可视化:所有被试均值的分布
```{r}
df.plot <- df.rt_m %>%
    mutate(
    condition = case_when(
                  condition == "TR_main" ~ "TaskRelevant",
                  condition == "TIR_main" ~ "TaskIrrelevant",
                ),
    ismatch = case_when(
      ismatch == "1" ~ "match",
      ismatch == "0" ~ "mismatch",
    ),
    shapes = factor(shapes, levels = c("自我", "朋友","生人")),
    )%>%
  dplyr::mutate(
    conds = case_when(
      ismatch == "match" & shapes == "自我" ~ "0.8",
      ismatch == "mismatch" & shapes == "自我" ~ "1.2",
      ismatch == "match" & shapes == "朋友" ~ "1.8",
      ismatch == "mismatch" & shapes == "朋友" ~ "2.2",
      ismatch == "match" & shapes == "生人" ~ "2.8",
      ismatch == "mismatch" & shapes == "生人" ~ "3.2",
    ),
    conds = as.numeric(conds)
  )


# 绘制各条件均值
plot_1 <- df.plot%>% #df.rt_m %>%
    # mutate(
    # condition = case_when(
    #               condition == "TR_main" ~ "TaskRelevant",
    #               condition == "TIR_main" ~ "TaskIrrelevant",
    #             ),
    # ismatch = case_when(
    #   ismatch == "1" ~ "match",
    #   ismatch == "0" ~ "mismatch",
    # ),
    # shapes = factor(shapes, levels = c("自我", "朋友","生人")),
    # )%>%
  ggplot2::ggplot(., 
                  aes(x = shapes,
                      y = RT_m,
                      group = ismatch,
                      color = ismatch)) +
  ggplot2::geom_line(position = position_dodge(0.9)) +
  ggplot2::geom_point(size = 3,
                      position = position_dodge(0.9)) + 
  ggplot2::geom_errorbar(aes(ymin=RT_m-RT_SE,
                             ymax=RT_m+RT_SE),
                         width=.1,
                         position = position_dodge(0.9)) +
  facet_wrap(~condition)+
  papaja::theme_apa()

# 各条件均值+每个被试散点数据
plot_2 <- plot_1 +
    ggplot2::geom_point(data = df.plot,
                       aes(x = conds, y = RT_m, group = as.factor(subj_idx)),
                      position = position_dodge(0.08),
                      color="#000000",
                      alpha = 0.05) +
  ggplot2::geom_line(data = df.plot,
                     aes(x = conds,
                         y = RT_m,
                         group = as.factor(subj_idx)),
                     position = position_dodge(0.08),
                     linetype = 1,
                     linewidth=0.8,
                     color="#000000",
                     alpha=0.05) +
  ggplot2::labs(y = "RT")+
  facet_wrap(~condition)

plot_2
# ggsave(file.path(figDir, "observed_data.png"))
```


# 分类任务反应时建模：这部分需要完善，等实验跑起来再说
## 模型拟合
```{r}
# fit a three-level hierarchical model for RT, didn't specify the prior, lognormal, effective coding
RT_model <- df.bayes %>%
  dplyr::filter(acc == 1) %>%
  dplyr::filter(ismatch == 1) %>%
  brms::brm(
    rt ~ condition * shapes +
      (1 + condition * shape | subj_idx),
    family = lognormal(),
    data = .,
    chains = 4,
    warmup = 1000,
    iter = 5000,
    thin = 1,
    control = list(adapt_delta = .95),
    cores = parallel::detectCores(),
    backend = 'cmdstanr',
    # with cmdstanr
    save_pars = save_pars(all = TRUE),
    file = file.path(curDir, "glmmModels/RT_3_level")
  )
```


## 模型评估
### MCMC采样评估：量化指标
```{r}
summary(RT_model)
```
### MCMC采样评估：轨迹图
```{r}
# 单独绘制轨迹图
p_trace <- mcmc_trace(RT_model, pars = c("b_Intercept", "b_conditionTaskRelevant","b_conditionTaskIrrelevant", "b_shapes朋友",
                           "b_shapes生人", "b_conditionTaskRelevant:shapes朋友", "b_conditionTaskIrrelevant:shapes朋友","b_conditionTaskRelevant:shapes生人","b_conditionTaskIrrelevant:shapes生人", "sigma"))
p_trace
ggsave(file.path(figDir, "trace_plot.png"))
```
```{r}
# 单独绘制采样分布

p_sample <- bayestestR::estimate_density(RT_model)
plot(p_sample)
ggsave(file.path(figDir, "parameters_distribution.png"))

# 同时绘制采样分布与轨迹图
# p_dens <- mcmc_plot(RT_model, type = "dens", pars = "^b_")
# p_trace <- mcmc_plot(RT_model, type = "trace", pars = "^b_")
# p_dens + p_trace
```


### 模型评估
```{r}
# performance::check_predictions(RT_model, check_range = TRUE)
bayesplot::pp_check(RT_model, type = "dens_overlay", ndraws = 1000)
ggsave(file.path(figDir, "modle_estimation.png"))
```

```{r}
plot(conditional_effects(RT_model),plot = FALSE)[3]
```


## 推断统计：参数显著性检验

需要观察HDI是否包含0，条件间差异是否不等于0

### 各条件rt计算与可视化
```{r}
# 计算所有参数均值和hdi，就是summary里的固定效应
pop_mean <- RT_model %>%
        tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%   # 使用正则表达式，提取b_开头的后验分布值
        group_by(.variable) %>%       # this line not necessary (done automatically by spread_draws)
        tidybayes::mean_hdci(.value) # 计算所有参数的均值和高密度连续区间（HDI）

```

#暂未根据实验设计修改
```{r}
# 提取总体水平的后验参数
df_rt_plot <- RT_model %>%
  tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%
  dplyr::rename(term = .variable,
                pop_mean = .value) %>%  # gather_draws会将变量存储为1列，所有的值存储为另一列
  dplyr::ungroup() %>%
  dplyr::mutate(term = gsub("b_", "", term))%>%
  tidyr::pivot_wider(names_from = c(term), values_from = pop_mean) %>%   # long to wide
  # 计算各条件反应时分布
  dplyr::mutate(
    Self_PSelf = Intercept,   #  self和classify_self被编码为0              
    Friend_PSelf = Intercept  + shape朋友,
    Stranger_PSelf = Intercept  + shape生人,
    Self_PFriend = Intercept + conditionclassify_friend,             
    Friend_PFriend = Intercept + conditionclassify_friend + shape朋友 + `conditionclassify_friend:shape朋友`,
    Stranger_PFriend = Intercept + conditionclassify_friend + shape生人 + `conditionclassify_friend:shape生人`,
    Self_PStranger = Intercept + conditionclassify_stranger,             
    Friend_PStranger = Intercept + conditionclassify_stranger + shape朋友 + `conditionclassify_stranger:shape朋友`,
    Stranger_PStranger = Intercept + conditionclassify_stranger + shape生人 + `conditionclassify_stranger:shape生人`)%>%
  dplyr::select(`.chain`, `.iteration`, `.draw`,
                Self_PSelf, Friend_PSelf, Stranger_PSelf,
                Self_PFriend, Friend_PFriend, Stranger_PFriend,
                Self_PStranger, Friend_PStranger, Stranger_PStranger) %>%
  # wide to long
  tidyr::pivot_longer(cols = Self_PSelf:Stranger_PStranger, names_to = "term", values_to =  "value") %>%  
  dplyr::mutate(term = factor(term, levels = c(
                'Self_PSelf', 'Friend_PSelf', 'Stranger_PSelf',
                'Self_PFriend', 'Friend_PFriend', 'Stranger_PFriend',
                'Self_PStranger', 'Friend_PStranger', 'Stranger_PStranger'
  )),
  # log还原
  value = exp(value))

```

```{r}
# 描述固定效应
df_rt_fixed_effect <- df_rt_plot %>%
        tidyr::pivot_wider(.,
                           id_cols = c('.chain', '.iteration', '.draw'),
                           names_from = term ) %>% 
        dplyr::select(-c('.chain', '.iteration', '.draw')) %>%
        bayestestR::describe_posterior(.,
                                       ci = 0.95,
                                       ci_method = 'hdi',
                                       test = c("p_direction", "p_significance"),
                                       centrality = "median") %>%
        tidyr::separate(Parameter, c('shape', 'condition')) %>% 
        dplyr::mutate(shape = factor(shape, levels = c('Self', 'Friend', 'Stranger')),
                      condition = factor(condition, levels = c('PSelf', 'PFriend', 'PStranger')))

# use the overall mean values as the vlines
vlines <- df_rt_plot %>%
  tidyr::separate(term, c('shape', 'condition')) %>%
  dplyr::mutate(shape = factor(shape, levels = c('Self', 'Friend', 'Stranger')),
                condition = factor(condition, levels = c('PSelf', 'PFriend', 'PStranger'))) %>%
  dplyr::group_by(shape, condition) %>% 
  dplyr::summarize(Mean = mean(value)) # %>%
        # dplyr::arrange(Mean)

```

```{r}
# THIS is the one which the final plot will based on!!!
p_rt1 <- df_rt_plot %>%
  tidyr::separate(term, c('shape', 'condition')) %>% 
  dplyr::mutate(shape = factor(shape, levels = c('Self', 'Friend', 'Stranger')),
                      condition = factor(condition, levels = c('PSelf', 'PFriend', 'PStranger')))%>%
  # 表示不同实验条件下，shape对反应时的影响  value=RT
  ggplot2::ggplot(aes(x = value, color = shape)) +
  tidybayes::stat_halfeye(aes(fill = shape), alpha = 0.6) +
  geom_vline(data = vlines, aes(xintercept = Mean,color = shape), linetype = "dashed") +
  facet_wrap(~ condition, nrow = 1) +
  labs(x=expression('Posteior distribution of reaction times'))
  # scale_colour_brewer(palette = "Dark2") +
  # scale_fill_brewer(palette = "Dark2") +
  # theme_apa(base_size = 10)

p_rt1

```

```{r}
# 参数>0或<0的比例
RR_PD <- bayestestR::p_direction(RT_model)
RR_PD
plot(RR_PD)
```

```{r}
# 参数不为0的比例
RR_PS <- bayestestR::p_significance(RT_model)
RR_PS
x <- plot(RR_PS)
# 提取颜色
g <- ggplot2::ggplot_build(x)
unique(g$data[[1]]$fill)
```
```{r}
# HDI 是否包含0
RT_hdi <- bayestestR::hdi(RT_model, ci = 0.95)

RT_hdi

plot(RT_hdi) + scale_fill_flat()
```
```{r}
# HDI是否包含0，另一种表现方式
b <- parameters::model_parameters(RT_model, effects = "fixed", component = "conditional")

plot(b, show_labels = TRUE, size_text = 4)

ggsave(file.path(figDir, "parameter_significance.png"))
```



### 条件间rt差异计算与可视化
```{r}
# 计算条件间差异
df_rt_plot_diff <- df_rt_plot %>%
  tidyr::pivot_wider(names_from = c(term), values_from = value) %>%
  dplyr::mutate(
    # 按照目标优势的方式写
    # diff_SF_PS = Self_PSelf - Friend_PSelf,
    # diff_SStr_PS = Self_PSelf - Stranger_PSelf ,
    # diff_FS_PS = Friend_PSelf - Stranger_PSelf ,
    # diff_FS_PF = Friend_PFriend - Self_PFriend,
    # diff_FStr_PF = Friend_PFriend - Stranger_PFriend,
    # diff_SStr_PF = Self_PFriend - Stranger_PFriend,
    # diff_StrS_PStr = Stranger_PStranger - Self_PStranger,
    # diff_StrF_PStr = Stranger_PStranger - Friend_PStranger ,
    # diff_SF_PStr = Self_PStranger - Friend_PStranger
    # 按照自我优势收到目标调节的方式写
    diff_SF_PS = Self_PSelf - Friend_PSelf,
    diff_SStr_PS = Self_PSelf - Stranger_PSelf ,
    diff_FStr_PS = Friend_PSelf - Stranger_PSelf ,
    diff_SF_PF = Self_PFriend - Friend_PFriend,
    diff_SStr_PF = Self_PFriend - Stranger_PFriend ,
    diff_FStr_PF = Friend_PFriend - Stranger_PFriend ,
    diff_SF_PStr = Self_PStranger - Friend_PStranger,
    diff_SStr_PStr = Self_PStranger - Stranger_PStranger ,
    diff_FStr_PStr = Friend_PStranger - Stranger_PStranger 
  ) %>%
  dplyr::select(
    `.chain`,
    `.iteration`,
    `.draw`,
    diff_SF_PS,
    diff_SStr_PS,
    diff_FStr_PS,
    
    diff_SF_PF,
    diff_SStr_PF,
    diff_FStr_PF,
    
    diff_SF_PStr,
    diff_SStr_PStr,
    diff_FStr_PStr
  ) %>%
  tidyr::pivot_longer(
    cols = diff_SF_PS:diff_FStr_PStr,
    names_to = "term_diff",
    values_to =  "value"
  ) %>%  # wide to long
  dplyr::mutate(term_diff = factor(
    term_diff,
    levels = c(
      'diff_SF_PS',
      'diff_SStr_PS',
      'diff_FStr_PS',
      
      'diff_SF_PF',
      'diff_SStr_PF',
      'diff_FStr_PF',
      
      'diff_SF_PStr',
      'diff_SStr_PStr',
      'diff_FStr_PStr'
    )
  ))

```

```{r}
# RT 系数显著大于0的部分
df_rt1_diff_hdi <- df_rt_plot_diff %>%
  tidyr::pivot_wider(.,
                     id_cols = c('.chain', '.iteration', '.draw'),
                     names_from = term_diff) %>%
  dplyr::select(-c('.chain', '.iteration', '.draw')) %>%
  bayestestR::describe_posterior(
    .,
    ci = 0.95,
    ci_method = 'hdi',
    test = c("p_direction", "p_significance"),
    centrality = "median"
  )


```

```{r}
p_rt1_diff <- df_rt_plot_diff %>%
  # 表示不同实验条件下反应时的差异，value表示条件间反应时的差异
  ggplot2::ggplot(aes(x = value, fill = after_stat(x < 0))) +
  tidybayes::stat_halfeye() +
  geom_vline(xintercept = 0, linetype = "dashed") + # 添加x=0的垂线
  scale_fill_manual(
    values = c('#2980B9', '#E74C3C'),# 蓝色出现在true组，红色出现在false组
    breaks = c(TRUE, FALSE),
    name = "Effect" ,
    labels = c("<0", ">0")# 标签yes表示x<0
  ) +
  xlab("Effect of shape on RT") +
  facet_wrap( ~ term_diff, nrow = 3, ) +
  theme_modern()


# theme_apa(base_size = 20) +

# theme(strip.text.x = element_text(size = 8)) # colour = "orange", angle = 90))


p_rt1_diff
ggsave(file.path(figDir, "diff_compare.png"))
```

# ACC 建模

## 初步可视化
```{r}
df.c.acc.sum <- df.c.acc.subj %>% 
  dplyr::group_by(condition, shape) %>%
  dplyr::summarise(grand_mean_acc = mean(ACC),
                   SD_acc = sd(ACC),
                   SE_acc = SD_acc/sqrt(n()-1),
                   n = n()) %>%
  dplyr::ungroup()

df.c.acc.plot <- df.c.acc.subj %>%
  dplyr::mutate(
    conds = case_when(
      condition == "classify_self" & shape == "自我" ~ "0.7",
      condition == "classify_self" & shape == "朋友" ~ "1",
      condition == "classify_self" & shape == "生人" ~ "1.3",
      condition == "classify_friend" & shape == "自我" ~ "1.7",
      condition == "classify_friend" & shape == "朋友" ~ "2",
      condition == "classify_friend" & shape == "生人" ~ "2.3",
      condition == "classify_stranger" & shape == "自我" ~ "2.7",
      condition == "classify_stranger" & shape == "朋友" ~ "3",
      condition == "classify_stranger" & shape == "生人" ~ "3.3",
    ),
    conds = as.numeric(conds)
  )

# 绘制各条件均值
plot_acc_1 <- df.c.acc.sum %>%
  ggplot2::ggplot(., 
                  aes(x = condition,
                      y = grand_mean_acc,
                      group = shape,
                      color = shape)) +
  ggplot2::geom_line(position = position_dodge(0.9)) +
  ggplot2::geom_point(size = 3,
                      position = position_dodge(0.9)) + 
  ggplot2::geom_errorbar(aes(ymin=grand_mean_acc-SE_acc,
                             ymax=grand_mean_acc+SE_acc),
                         width=.1,
                         position = position_dodge(0.9)) +
  papaja::theme_apa()

# 各条件均值+每个被试散点数据
plot_acc_2 <- plot_acc_1 +
    ggplot2::geom_point(data = df.c.acc.plot,
                       aes(x = conds, y = ACC, group = as.factor(subj_idx)),
                      position = position_dodge(0.08),
                      color="#000000",
                      alpha = 0.05) +
  ggplot2::geom_line(data = df.c.acc.plot,
                     aes(x = conds,
                         y = ACC,
                         group = as.factor(subj_idx)),
                     position = position_dodge(0.08),
                     linetype = 1,
                     linewidth=0.8,
                     color="#000000",
                     alpha=0.05) +
  ggplot2::labs(y = "ACC")

plot_acc_2

```

```{r}
# fit a three-level hierarchical model for RT, didn't specify the prior, lognormal, effective coding
acc_m1 <- df.c %>%
        brms::brm(acc ~ condition*shape + 
                          (1 + condition*shape | subj_idx),
                  family = bernoulli(link="probit"),
                  data = .,
                  chains = 4,
                  warmup = 1000, 
                  iter = 5000,
                  thin = 1,
                  control = list(adapt_delta = .95),
                  cores = parallel::detectCores(),
                  backend = 'cmdstanr',  # with cmdstanr
                  save_pars = save_pars(all = TRUE),
                  file = file.path(curDir, "glmmModels/CT_ACC_3_level"))
```
## 模型评估
```{r}
summary(acc_m1)
```

```{r}
mcmc_trace(acc_m1, pars = c("b_Intercept", "b_conditionclassify_friend","b_conditionclassify_stranger", "b_shape朋友",
                           "b_shape生人", "b_conditionclassify_friend:shape朋友", "b_conditionclassify_stranger:shape朋友","b_conditionclassify_friend:shape生人","b_conditionclassify_stranger:shape生人"))
```

```{r}
bayesplot::pp_check(acc_m1, type = "dens_overlay")
```

## 推断统计
```{r}
plot(conditional_effects(acc_m1),ask=FALSE)
```

### 各条件ACC分布
```{r}
# 提取总体水平的后验参数
df_acc_plot <- acc_m1 %>%
  tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%
  dplyr::rename(term = .variable,
                pop_mean = .value) %>%  # gather_draws会将变量存储为1列，所有的值存储为另一列
  dplyr::ungroup() %>%
  dplyr::mutate(term = gsub("b_", "", term))%>%
  tidyr::pivot_wider(names_from = c(term), values_from = pop_mean) %>%   # long to wide
  # 计算各条件ACC分布
  dplyr::mutate(
    Self_PSelf = Intercept,   #  self和classify_self被编码为0              
    Friend_PSelf = Intercept  + shape朋友,
    Stranger_PSelf = Intercept  + shape生人,
    Self_PFriend = Intercept + conditionclassify_friend,             
    Friend_PFriend = Intercept + conditionclassify_friend + shape朋友 + `conditionclassify_friend:shape朋友`,
    Stranger_PFriend = Intercept + conditionclassify_friend + shape生人 + `conditionclassify_friend:shape生人`,
    Self_PStranger = Intercept + conditionclassify_stranger,             
    Friend_PStranger = Intercept + conditionclassify_stranger + shape朋友 + `conditionclassify_stranger:shape朋友`,
    Stranger_PStranger = Intercept + conditionclassify_stranger + shape生人 + `conditionclassify_stranger:shape生人`)%>%
  dplyr::select(`.chain`, `.iteration`, `.draw`,
                Self_PSelf, Friend_PSelf, Stranger_PSelf,
                Self_PFriend, Friend_PFriend, Stranger_PFriend,
                Self_PStranger, Friend_PStranger, Stranger_PStranger) %>%
  # wide to long
  tidyr::pivot_longer(cols = Self_PSelf:Stranger_PStranger, names_to = "term", values_to =  "value") %>%  
  dplyr::mutate(term = factor(term, levels = c(
                'Self_PSelf', 'Friend_PSelf', 'Stranger_PSelf',
                'Self_PFriend', 'Friend_PFriend', 'Stranger_PFriend',
                'Self_PStranger', 'Friend_PStranger', 'Stranger_PStranger'
  )))

```

```{r}
# 描述固定效应
df_acc_fixed_effect <- df_acc_plot %>%
        tidyr::pivot_wider(.,
                           id_cols = c('.chain', '.iteration', '.draw'),
                           names_from = term ) %>% 
        dplyr::select(-c('.chain', '.iteration', '.draw')) %>%
        bayestestR::describe_posterior(.,
                                       ci = 0.95,
                                       ci_method = 'hdi',
                                       test = c("p_direction", "p_significance"),
                                       centrality = "median") %>%
        tidyr::separate(Parameter, c('shape', 'condition')) %>% 
        dplyr::mutate(shape = factor(shape, levels = c('Self', 'Friend', 'Stranger')),
                      condition = factor(condition, levels = c('PSelf', 'PFriend', 'PStranger')))

# use the overall mean values as the vlines
vlines <- df_acc_plot %>%
  tidyr::separate(term, c('shape', 'condition')) %>%
  dplyr::mutate(shape = factor(shape, levels = c('Self', 'Friend', 'Stranger')),
                condition = factor(condition, levels = c('PSelf', 'PFriend', 'PStranger'))) %>%
  dplyr::group_by(shape, condition) %>% 
  dplyr::summarize(Mean = mean(value)) # %>%
        # dplyr::arrange(Mean)

```

```{r}
# THIS is the one which the final plot will based on!!!
p_acc1 <- df_acc_plot %>%
  tidyr::separate(term, c('shape', 'condition')) %>% 
  dplyr::mutate(shape = factor(shape, levels = c('Self', 'Friend', 'Stranger')),
                      condition = factor(condition, levels = c('PSelf', 'PFriend', 'PStranger')))%>%
  # 表示不同实验条件下，shape对反应时的影响  value=RT
  ggplot2::ggplot(aes( x = value, color = shape)) +
  tidybayes::stat_halfeye(aes(fill = shape), alpha = 0.6) +
  geom_vline(data = vlines, aes(xintercept = Mean,color = shape), linetype = "dashed") +
  facet_wrap(~ condition, nrow = 1) +
  labs(x=expression('Posteior distribution of accuracy'))
  # scale_colour_brewer(palette = "Dark2") +
  # scale_fill_brewer(palette = "Dark2") +
  # theme_apa(base_size = 10)

p_acc1

```




### 各条件ACC差异比较
```{r}
# 计算条件间差异
df_acc_plot_diff <- df_acc_plot %>%
  tidyr::pivot_wider(names_from = c(term), values_from = value) %>%
  dplyr::mutate(
    diff_SF_PS = Self_PSelf - Friend_PSelf,
    diff_SStr_PS = Self_PSelf - Stranger_PSelf ,
    diff_FS_PS = Friend_PSelf - Stranger_PSelf ,
    diff_FS_PF = Friend_PFriend - Self_PFriend,
    diff_FStr_PF = Friend_PFriend - Stranger_PFriend,
    diff_SStr_PF = Self_PFriend - Stranger_PFriend,
    diff_StrS_PStr = Stranger_PStranger - Self_PStranger,
    diff_StrF_PStr = Stranger_PStranger - Friend_PStranger ,
    diff_SF_PStr = Self_PStranger - Friend_PStranger
  ) %>%
  dplyr::select(
    `.chain`,
    `.iteration`,
    `.draw`,
    diff_SF_PS,
    diff_SStr_PS,
    diff_FS_PS,
    diff_FS_PF,
    diff_FStr_PF,
    diff_SStr_PF,
    diff_StrS_PStr,
    diff_StrF_PStr,
    diff_SF_PStr
  ) %>%
  tidyr::pivot_longer(
    cols = diff_SF_PS:diff_SF_PStr,
    names_to = "term_diff",
    values_to =  "value"
  ) %>%  # wide to long
  dplyr::mutate(term_diff = factor(
    term_diff,
    levels = c(
      'diff_SF_PS',
      'diff_SStr_PS',
      'diff_FS_PS',
      'diff_FS_PF',
      'diff_FStr_PF',
      'diff_SStr_PF',
      'diff_StrS_PStr',
      'diff_StrF_PStr',
      'diff_SF_PStr'
    )
  ))

```

```{r}
# RT 系数显著大于0的部分
df_acc_diff_hdi <- df_acc_plot_diff %>%
  tidyr::pivot_wider(.,
                     id_cols = c('.chain', '.iteration', '.draw'),
                     names_from = term_diff) %>%
  dplyr::select(-c('.chain', '.iteration', '.draw')) %>%
  bayestestR::describe_posterior(
    .,
    ci = 0.95,
    ci_method = 'hdi',
    test = c("p_direction", "p_significance"),
    centrality = "median"
  )

```

```{r}
p_acc1_diff <- df_acc_plot_diff %>%
  # 表示不同实验条件下反应时的差异，value表示条件间反应时的差异
  ggplot2::ggplot(aes(x = value, fill = after_stat(x > 0))) +
  tidybayes::stat_halfeye() +
  geom_vline(xintercept = 0, linetype = "dashed") + # 添加x=0的垂线
  scale_fill_manual(
    values = c('#2980B9', '#E74C3C'),# 蓝色出现在true组，红色出现在false组
    breaks = c(TRUE, FALSE),
    name = "Effect" ,
    labels = c("Yes", "No")# 标签yes表示x<0
  ) +
  xlab("Effect of shape on RT") +
  facet_wrap( ~ term_diff, nrow = 3, ) +
  theme_modern()


# theme_apa(base_size = 20) +

# theme(strip.text.x = element_text(size = 8)) # colour = "orange", angle = 90))


p_acc1_diff
```

# 结果报告
```{r}
# report::report(RT_model)
report::report(sessionInfo())
```

<!-- # 分类任务(不适合用dprime) -->
<!-- ## 数据准备 -->
<!-- ```{r} -->
<!-- df.c.self <- df.c%>% -->
<!--   dplyr::filter(condition == 'classify_self')%>% -->
<!--   dplyr::mutate( -->
<!--     isself = ifelse(shape == '自我', 1, 0), -->
<!--     sayself = ifelse((shape == '自我' & acc == 1) | (shape != '自我' & acc == 0), 1, 0), -->
<!--     shape = factor(shape, levels = c("自我", "朋友", "生人")), -->
<!--   )%>% -->
<!--   select(subj_idx, shape, isself, sayself) -->

<!-- df.c.self -->
<!-- ``` -->

<!-- ## 模型拟合 -->
<!-- ```{r} -->
<!-- # 大概跑了4h,有效样本量是个位数。 -->
<!-- sdt_self_m1 <- brms::brm(sayself ~ 0 + shape + shape:isself +  -->
<!--                                 (0 + shape + shape:isself | subj_idx), -->
<!--                         family = bernoulli(link="probit"),  -->
<!--                         data = df.c.self, -->
<!--                         chains = 4, -->
<!--                         warmup = 1000, -->
<!--                         iter = 10000,  -->
<!--                         thin = 2,  -->
<!--                         control = list(adapt_delta = .95), -->
<!--                         cores = parallel::detectCores(), -->
<!--                         backend = 'cmdstanr', -->
<!--                         save_pars = save_pars(all = TRUE), -->
<!--                         file = file.path(curDir, "glmmModels/sdt_self_m1") -->
<!--                         ) -->
<!-- ``` -->

<!-- ## 模型评估 -->
<!-- ```{r} -->
<!-- summary(sdt_self_m1) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- # # Total execution time: 6325.2 seconds.1.757h  为啥5000采样有效样本量这么少，模型有问题吗?还是采样步长有问题？当前正尝试步长为2并扩大采样次数看看 -->

<!-- # sdt_self_m1 <- brms::brm(sayself ~ 0 + shape + shape:isself +  -->
<!-- #                                 (0 + shape + shape:isself | subj_idx), -->
<!-- #                         family = bernoulli(link="probit"),  -->
<!-- #                         data = df.c.self, -->
<!-- #                         chains = 4, -->
<!-- #                         warmup = 1000, -->
<!-- #                         iter = 5000,  -->
<!-- #                         thin = 1,  -->
<!-- #                         control = list(adapt_delta = .95), -->
<!-- #                         cores = parallel::detectCores(), -->
<!-- #                         backend = 'cmdstanr', -->
<!-- #                         save_pars = save_pars(all = TRUE), -->
<!-- #                         file = file.path(curDir, "glmmModels/sdt_self_m1") -->
<!-- #                         ) -->

<!-- #  Family: bernoulli  -->
<!-- #   Links: mu = probit  -->
<!-- # Formula: sayself ~ 0 + shape + shape:isself + (0 + shape + shape:isself | subj_idx)  -->
<!-- #    Data: df.c.self (Number of observations: 6313)  -->
<!-- #   Draws: 4 chains, each with iter = 5000; warmup = 1000; thin = 1; -->
<!-- #          total post-warmup draws = 16000 -->
<!-- #  -->
<!-- # Multilevel Hyperparameters: -->
<!-- # ~subj_idx (Number of levels: 20)  -->
<!-- #                                        Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS -->
<!-- # sd(shape自我)                              0.27      0.18     0.00     0.66 1.45        8       16 -->
<!-- # sd(shape朋友)                              0.49      0.09     0.34     0.70 1.07       50       91 -->
<!-- # sd(shape生人)                              0.28      0.07     0.17     0.43 1.09       50       69 -->
<!-- # sd(shape自我:isself)                       0.30      0.16     0.05     0.70 1.71        6       40 -->
<!-- # sd(shape朋友:isself)                       2.64      4.16     0.15     8.98 1.01       95      224 -->
<!-- # sd(shape生人:isself)                       2.32      2.88     0.06     9.40 1.13       21       45 -->
<!-- # cor(shape自我,shape朋友)                  -0.11      0.44    -0.75     0.81 2.13        5       17 -->
<!-- # cor(shape自我,shape生人)                  -0.32      0.36    -0.87     0.37 1.63        7       27 -->
<!-- # cor(shape朋友,shape生人)                   0.36      0.24    -0.14     0.80 1.20       15       27 -->
<!-- # cor(shape自我,shape自我:isself)           -0.22      0.35    -0.83     0.53 1.14       27       79 -->
<!-- # cor(shape朋友,shape自我:isself)           -0.37      0.39    -0.87     0.55 1.20       15       25 -->
<!-- # cor(shape生人,shape自我:isself)           -0.31      0.33    -0.80     0.52 1.30       11       55 -->
<!-- # cor(shape自我,shape朋友:isself)            0.04      0.37    -0.65     0.75 1.07       68       79 -->
<!-- # cor(shape朋友,shape朋友:isself)            0.01      0.39    -0.83     0.72 1.20       15       53 -->
<!-- # cor(shape生人,shape朋友:isself)            0.07      0.37    -0.66     0.72 1.11       32      239 -->
<!-- # cor(shape自我:isself,shape朋友:isself)    -0.05      0.38    -0.73     0.71 1.05       57       70 -->
<!-- # cor(shape自我,shape生人:isself)            0.02      0.39    -0.70     0.72 1.03       84      204 -->
<!-- # cor(shape朋友,shape生人:isself)            0.03      0.36    -0.63     0.67 1.01      120      398 -->
<!-- # cor(shape生人,shape生人:isself)            0.01      0.36    -0.66     0.68 1.02      105      230 -->
<!-- # cor(shape自我:isself,shape生人:isself)     0.02      0.38    -0.69     0.72 1.02       66      125 -->
<!-- # cor(shape朋友:isself,shape生人:isself)    -0.01      0.39    -0.76     0.75 1.04       90      141 -->
<!-- #  -->
<!-- # Regression Coefficients: -->
<!-- #                   Estimate Est.Error  l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS -->
<!-- # shape自我          -147.36    458.73   -878.56   809.52 2.32        5       11 -->
<!-- # shape朋友            -1.30      0.10     -1.52    -1.12 1.18       15       24 -->
<!-- # shape生人            -1.33      0.07     -1.47    -1.19 1.04       57      101 -->
<!-- # shape自我:isself    148.73    458.71   -808.08   879.64 2.32        5       11 -->
<!-- # shape朋友:isself   5486.03   4711.51  -3908.32 14075.66 1.71        6       20 -->
<!-- # shape生人:isself -10328.61  14632.45 -52039.36  2957.96 1.86        6       17 -->
<!-- #  -->
<!-- # Draws were sampled using sample(hmc). For each parameter, Bulk_ESS -->
<!-- # and Tail_ESS are effective sample size measures, and Rhat is the potential -->
<!-- # scale reduction factor on split chains (at convergence, Rhat = 1). -->
<!-- ``` -->


<!-- ```{r} -->
<!-- mcmc_trace(sdt_self_m1, pars = c( "b_shape自我","b_shape朋友","b_shape生人", "b_shape自我:isself", "b_shape朋友:isself","b_shape生人:isself")) -->
<!-- ``` -->

<!-- ## 各条件d值计算与可视化 -->
<!-- ```{r} -->
<!-- # 描述参数后验分布 -->
<!-- df_std_self_fixed_effect <- bayestestR::describe_posterior( -->
<!--         sdt_self_m1, -->
<!--         effects = "fixed",        # 只分析固定效应 -->
<!--         component = "all",        # 分析均值和方差 -->
<!--         ci = 0.95, -->
<!--         ci_method = 'hdi',        # 可信区间方法为最高密度区间 -->
<!--         # 计算参数值大于零的概率（p_direction）计算参数值显著不为零的概率（p_significance） -->
<!--         test = c("p_direction", "p_significance"),        -->
<!--         centrality = "all") %>% # 计算所有集中趋势指标，包括均值、中位数和众数。 -->
<!--         dplyr::mutate(shape = dplyr::case_when( -->
<!--                 grepl("shape自我", Parameter) ~ "self", -->
<!--                 grepl("shape朋友", Parameter) ~ "friend", -->
<!--                 grepl("shape生人", Parameter) ~ "stranger"), -->
<!--                 # 敏感性（d'）：反映了被试对信号的感知能力，是一个客观的指标，不受被试决策偏好的影响。 -->
<!--                 # 判断标准（criterion）：反映了被试的决策偏好，是一个主观的指标，可能受到任务难度、奖励机制等因素的影响。 -->
<!--                 params = dplyr::case_when(grepl("isself", Parameter) ~ "d prime",   # 依赖于前期将match编码为1 -->
<!--                                            !grepl("isself", Parameter) ~"criterion"), -->
<!--                 params = factor(params, levels = c('d prime', 'criterion')), -->
<!--                 shape = factor(shape, levels = c('self', 'friend',  'stranger')))  -->

<!-- # 计算固定效应均值和hdi，summary里有 -->
<!-- self_pop_mean <- sdt_self_m1 %>% -->
<!--         tidybayes::gather_draws(`b_.*`, regex = TRUE) %>%   # 使用正则表达式，提取b_开头的后验分布值，所有参数都是b_开头 -->
<!--         group_by(.variable) %>%       # this line not necessary (done automatically by spread_draws) -->
<!--         tidybayes::mean_hdci(.value)  # 计算所有参数的均值和高密度连续区间（HDI） -->

<!-- # 提取固定效应估计的参数值 -->
<!-- df_sdt_self_pop <- sdt_self_m1 %>%  -->
<!--         tidybayes::gather_draws(`b_.*`, regex = TRUE) %>% -->
<!--         dplyr::rename(term = .variable, -->
<!--                       pop_mean = .value) %>%  # gather_draws会将变量存储为1列，所有的值存储为另一列 -->
<!--         dplyr::ungroup() %>% -->
<!--         dplyr::mutate(term = gsub("b_", "", term))   # 使用正则表达式去掉b_ -->



<!-- # 计算不同实验条件d值后验分布 -->
<!-- df_sdt_self_plot <- df_sdt_self_pop %>% -->
<!--         dplyr::select(term, `.chain`, `.iteration`, `.draw`, pop_mean) %>% -->
<!--         dplyr::mutate(                    # c_表示反应倾向，dprime表示敏感性，match为信号，mismatch为噪声 -->
<!--                       term = dplyr::case_when((term == "shape自我") ~ "c_self", -->
<!--                                               (term == "shape朋友") ~ "c_friend", -->
<!--                                               (term == "shape生人") ~ "c_stranger", -->
<!--                                               (term == "shape自我:isself") ~ "dprime_self",   # match为1,misself为0 -->
<!--                                               (term == "shape朋友:isself") ~ "dprime_friend", -->
<!--                                               (term == "shape生人:isself") ~ "dprime_stranger"), -->
<!--                       term = factor(term, levels = c("c_self", "c_friend", "c_stranger", -->
<!--                                                      "dprime_self", "dprime_friend", "dprime_stranger")))  -->

<!-- vlines_df_m1_sdt <- df_sdt_self_plot %>%  -->
<!--         tidyr::separate(term, c('params', 'shape')) %>% # 用于将一个字符型变量拆分为多个变量 -->
<!--         dplyr::mutate(shape = factor(shape, levels = c('self', 'friend', 'stranger'))) %>% -->
<!--         dplyr::filter(params == 'dprime') %>% -->
<!--         dplyr::group_by(shape) %>%  -->
<!--         dplyr::summarize(Mean = mean(pop_mean))  -->

<!-- # 不同实验条件d值后验分布可视化 -->
<!-- p_sdt_self <- df_sdt_self_plot %>% -->
<!--         tidyr::separate(term, c('params', 'shape')) %>% -->
<!--         dplyr::mutate(shape = factor(shape, levels = c('self', 'friend', 'stranger'))) %>% -->
<!--         dplyr::filter(params == 'dprime') %>% -->
<!--         ggplot2::ggplot(aes(x = pop_mean, color = shape)) +# value是各系数的后验估计值 -->
<!--         tidybayes::stat_halfeye(aes(fill = shape), alpha = 0.7) + -->
<!--         geom_vline(data = vlines_df_m1_sdt, aes(xintercept = Mean,colour = shape), linetype = "dashed") + -->
<!--         labs(x=expression('Posteior distribution of '~italic(d)~' prime')) +  -->
<!--         scale_colour_brewer(palette = "Dark2") + -->
<!--         scale_fill_brewer(palette = "Dark2") + -->
<!--         theme_apa(base_size = 20) -->
<!-- p_sdt_self -->
<!-- ``` -->

<!-- ## 条件间d值差异计算与可视化 -->
<!-- ```{r} -->
<!-- # 计算各条件d值差异--宽数据 -->
<!-- df_sdt_self_plot_diff_wide <- df_sdt_self_plot %>% -->
<!--   tidyr::pivot_wider(names_from = c(term), values_from = pop_mean) %>% -->
<!--   dplyr::mutate(diff_SF_c = c_self - c_friend,                           -->
<!--                 diff_SSTR_c = c_self - c_stranger, -->
<!--                 diff_FSTR_c = c_friend - c_stranger, -->
<!--                 diff_SF_dprm = dprime_self - dprime_friend, -->
<!--                 diff_SSTR_dprm = dprime_self - dprime_stranger, -->
<!--                 diff_FSTR_dprm = dprime_friend - dprime_stranger) %>% -->
<!--   dplyr::select(`.chain`, `.iteration`, `.draw`, -->
<!--                diff_SF_c,diff_SSTR_c, diff_FSTR_c, -->
<!--                diff_SF_dprm, diff_SSTR_dprm, diff_FSTR_dprm) -->

<!-- # 计算各条件d值差异--长数据 -->
<!-- df_sdt_self_plot_diff <- df_sdt_self_plot_diff_wide%>% -->
<!--   tidyr::pivot_longer(cols = diff_SF_c:diff_FSTR_dprm, names_to = "term_diff", values_to =  "value") %>%  # wide to long -->
<!--   dplyr::mutate(term_diff = factor(term_diff, levels = c('diff_SF_c','diff_SSTR_c', 'diff_FSTR_c', -->
<!--                                                          'diff_SF_dprm', 'diff_SSTR_dprm', 'diff_FSTR_dprm'))) -->

<!-- # 各条件d值差异可视化 -->
<!-- p_sdt_self_diff <- df_sdt_self_plot_diff %>% -->
<!--         dplyr::filter(str_detect(term_diff, '_dprm')) %>% -->
<!--         # 表示不同实验下，与中性效价的敏感性差异，标记出x>0的部分 -->
<!--         ggplot2::ggplot(aes(x = value, fill = after_stat(x > 0))) +   # 选择与0比较的方向，取决于差异计算方式以及实验目的 -->
<!--         tidybayes::stat_halfeye() + -->
<!--         geom_vline(xintercept = 0, linetype = "dashed") + -->
<!--         # scale_fill_manual(values = c('gray80', 'skyblue')) + -->
<!--         scale_fill_manual(values = c('skyblue', 'gray80'), -->
<!--                     breaks = c(TRUE, FALSE), -->
<!--                     name = "Effect" , labels = c("Yes", "No")) +  -->
<!--         xlab(expression(paste("Effect of shape on ", italic("d"), " prime", sep = ' '))) +  -->
<!--         facet_wrap( ~ term_diff, # scales = "free_y", -->
<!--                     nrow = 1)+ -->
<!--         theme_apa(base_size = 10) +  -->
<!--         theme(strip.text.x = element_text(size = 8)) # colour = "orange", angle = 90)) -->
<!-- p_sdt_self_diff -->
<!-- ``` -->


